...
@Component({
  selector: 'app-profile-big',
  templateUrl: './big.component.html',
  styleUrls: ['./big.component.scss']
})
export class ProfileBigComponent implements OnInit, OnChanges {
  @Input() profile;
  @Input() listType;
  @Input() moJsReady;
  @Input() last;
  ...
  ngOnInit() {
    ...
    this.avatar = this.profile.featureUrl ? (this.config.prefix.wApi + this.profile.featureUrl) : (this.profile.headimageurl ? this.config.prefix.wApi + this.profile.headimageurl : '/assets/images/avatar.jpg');
  }
